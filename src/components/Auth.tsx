...
import { AtSign, ChevronLeft, ChevronRight, Github, KeyRound, LogOut } from "lucide-react";
...
const Auth = () => {
	...
	const [loginsOpen, setLoginsOpen] = useState<boolean>(false);
					...
					{loginsOpen && (
						<>
							<button onClick={loginSpotify} className="group py-0.5 px-3 flex flex-row gap-2 items-center bg-neutral-900 rounded-full">
								...
								<span className="text-neutral-300 group-hover:text-white transition-colors">Spotify</span>
							</button>
							<button onClick={loginGithub} className="group py-0.5 px-3 flex flex-row gap-2 items-center bg-neutral-900 rounded-full">
								...
								<span className="text-neutral-300 group-hover:text-white transition-colors">Github</span>
							</button>
						</>
					)}
