		...
		<div className="absolute md:w-96 md:left-5 md:top-5 right-2 left-2 top-2 bg-neutral-800/50 backdrop-blur-sm border border-spotify/75 hover:border-spotify rounded-lg transition-all group">
			<a href={`https://open.spotify.com/track/${trackID}`} target="_blank">
				<div className="relative p-2 flex flex-row gap-2">
					...
					<img className={`h-24 w-24 rounded ${albumCover ? "" : "animate-spin-slow"}`} src={albumCover ? albumCover : placeholder} alt="Album Cover" title={`${album ? album : "Failed to load"}`} />
					<div className="w-full flex flex-col justify-between">
						<div className="h-full flex flex-col justify-center">
							<p className="w-56 whitespace-nowrap text-ellipsis overflow-hidden text-white" title={song ? song : "Failed to load"}>
								{song ? song : "error"}
							</p>
							<p className="w-56 whitespace-nowrap text-ellipsis overflow-hidden text-neutral-400" title={artists ? artists : "Failed to load"}>
